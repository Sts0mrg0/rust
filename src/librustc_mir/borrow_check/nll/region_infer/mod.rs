...
impl<'tcx> RegionInferenceContext<'tcx> {
    ...
    fn init_universal_regions(&mut self) {
        ...
        for variable in universal_regions.universal_regions() {
            ...
            for point_index in elements.all_point_indices() {
                self.add_live_element(variable, point_index, &Cause::UniversalRegion(variable));
            }

            // Add `end(X)` into the set for X.
            self.add_live_element(variable, variable, &Cause::UniversalRegion(variable));
        }
    }
    ...
    pub(super) fn add_live_element(
        ...
        if self.liveness_constraints.add_element(v, elem, cause) {
            // ...but also add to the SCC in which `v` appears.
            let scc = self.constraints_scc.scc(v);
            self.scc_values.add_element(scc, elem, cause);
